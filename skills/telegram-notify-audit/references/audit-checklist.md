...
| workbuddy 免费模型列**具体模型名**而非只给计数 | `freeModels` 只是个数字，只写「免费模型 1」读者不知道是哪个。名字取自快照 `modelStates` 里 `costClass == "free"` 的**键**（键即模型名）。`costClass == "unknown"` **不算**免费，别把没测出来的当免费列出来 |
| 免费模型名清单**全量展示、不折叠** | 属**结构性清单**（规范 · 折叠规则：判据是「折掉会不会让读者误判」）：读者要逐条核对哪些模型免费，折叠成「还有 N 个…」等于把最需要看的部分藏起来。名字多时靠 `免费模型 N` 交代规模，并拆到独立子行避免单行过长 |
| 首轮探测换免费模型名（`/v1/models` + `/admin/probe`） | `modelStates` 只在模型被真实请求过时才有键（默认 `unknown`），光读快照拿不到名字。用 serve 自带 `/admin/probe`（**仅回环可调**、单次上限 50 个模型、结论写回账本并随快照持久化），**不另起进程直连上游**（账本在 serve 内存，独立进程写的文件会被覆盖）。只在首轮探，后续轮次靠 `restoreAccountRuntimeStateLocked` 恢复的账本 |
| 探测失败/超时时通知仍只给计数 | 拿不到名字就退回「免费模型 N」，**绝不编造模型名**。这是降级不是漏字段 |
| workbuddy TSV 的**第 1 列是探测标记** | `read` 会把连续制表符折叠成一个（制表符属 IFS 空白），中间列一旦为空，后面各列整体左移、错位嫁接到前一个变量上。把恒非空的探测标记放最前，就把错位挡在门外（此坑已实测复现） |
