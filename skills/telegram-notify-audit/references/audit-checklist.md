...
| workbuddy TSV 的**第 1 列是名录可信标记** | `read` 会把连续制表符折叠成一个（制表符属 IFS 空白），中间列一旦为空，后面各列整体左移、错位嫁接到前一个变量上。把恒非空的可信标记放最前，就把错位挡在门外（此坑已实测复现）。标记为 1 ⇔ 每个凭据都成功取到目录；不全成功时该列一律 0，渲染层据此决定「免费模型 0」写不写 |
