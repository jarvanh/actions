...
数字断言放在规范里必然腐化。本 skill 把核对流程固化下来：读真源 → 机械扫描 → 四域并行
通读 → **逐条复核** → 渲染预览 → 回归验证 → 回写基线；**用户贴了具体通知时，先本地
复现那条通知再动手**。
...
先完整读 `docs/telegram-notify.md`，重点是「版式构件」一章、折叠规则判定、
「发送层」（硬约束）、「验证与交付」；既有基线见 `references/audit-checklist.md`。
...
用户说「某条通知坏了」并贴出内容时，**不要先在脑子里推代码**——把那条通知在本地
原样复现出来，再对着渲染结果判断。三步：

1. mock 掉协作者（`rclone`、`send_telegram_message`、`_get_path_stats` 之类外部依赖），
   只留被测的渲染函数；
2. 直接调用渲染函数，把消息捕获进变量（`send_telegram_message() { CAPTURE="$1"; }`）
   再打印出来——**不发**；
3. 渲染结果与用户贴的内容逐行对齐，差异处即根因。

现成样板可抄 `tests/test_preview_diff.sh`：`source` 真源 → 定义 mock → 调
`add_preview_pair` / `flush_task_preview` → 读 `SEND_CAPTURE`。`render_preview.sh`
用的是同一手法，区别只是数据自造而非用户提供。

**为什么必须复现**：通知「自相矛盾」这类问题（前半句写「已重启容器复核」、后半句写
「未探测」），纯读代码很难判定是哪条路径产出的；复现一次，矛盾的两行同时出现在眼前，
根因立刻显形——2026-09-12 就是这么定位到「新增的后端写熔断短路了下游结论串」的。

**mock 的返回值必须与真函数字段数一致**（`_get_path_stats` 返
`bytes count human_size` 三段而非两段），否则渲染出的元数据错位，你会去查一个
不存在的 bug。
...
硬约束只有三处（发送层、收尾区接线、验证与交付）。
**「字符集受限、风险低」不是硬约束的豁免理由**——规范 · 发送层明写动态内容必须转义。
...
执行 `scripts/render_preview.sh`，用真源助手构造数据渲染一遍（现为 16 项自动校验）。
这一步能抓到纯代码审查漏掉的四类问题：转义被二次处理、空行数量不对、
`$( )` 吃掉尾换行导致条目粘连、二层列表的子行与条目正文不同列。

**新增校验项后必须做一次负向验证**：把被测代码临时退回错误形态，确认新校验变红——
否则不知道它是不是恒真（2026-09-12 给「二层列表」加 5 项校验时做过：退回旧的
6 字符 `tree_sub`，5 项全红）。
...
- `render_preview.sh` —— 用真源助手渲染预览（对应规范 · 渲染预览），直接执行即可。
