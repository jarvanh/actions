...
| 用途 | 路径 |
|---|---|
| 版式规范 | `docs/telegram-notify.md`（第 4 章构件速查、第 6 章硬约束、8.1 助手对照表） |
...
写之前先读 `docs/telegram-notify.md` 的第 4 章与第 6 章。只有三处是**硬约束**（第 6 章发送层、4.9 收尾区、第 7 章测试基线），其余是「建议 + 为什么 + 反例」——不要把风格建议说成军规，但硬约束没有例外条款。
...
- 状态 emoji 必须随结论降级：0 成功 / 数据缺失 → ⚠️ 或 ❌，不要恒 ✅（8.2 节）。
