...
| 用途 | 路径 |
|---|---|
| 版式规范 | `docs/telegram-notify.md`（版式构件、发送层硬约束、助手速查表） |
...
写之前先读 `docs/telegram-notify.md` 的「版式构件」与「发送层」两章。只有三处是**硬约束**（发送层、收尾区、验证与交付），其余是「建议 + 为什么 + 反例」——不要把风格建议说成军规，但硬约束没有例外条款。
...
- 状态 emoji 必须随结论降级：0 成功 / 数据缺失 → ⚠️ 或 ❌，不要恒 ✅（规范 · 状态图标）。
