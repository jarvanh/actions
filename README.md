...
⚠️ **该语义解读未被实测证实**：定向诊断 `35239688191`（对「已存在/不存在/刚建出」三种
目录分别发 mkdir）**三组都没复现 409**。注意该诊断打的是**手选的 `wopan175`**，与主轮
故障所在的 `wopan176Crypt` 系**不同后端/时段**，不能互相推断 ⇒ 409 的触发条件仍未知。
开关保留是因为它的放行分支有 `lsd` 兜底（不会误放行"目录其实不存在"），属保守判据。
