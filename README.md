...
🔑 **★★ 最高优先级：预检时序本身就是错的（2026-09-18 实测，run `35296822507`）**：
常规探针实测 `重启后写入: +0s=FAIL() +10s=OK +30s=OK +60s=OK`，
`列表可见性: 首次非空=10s` ⇒ **后端重启后约 10 秒才就绪**。
而 `_fix_probe_dir_writable` 是「重启容器 → **立即**读探针 → 读不到即判不可写」
（`file_fix.sh` 重启后无任何等待）⇒ 生产里
`🔀 原目录不可写（已重启确认）→ 跳过原目录的 4 种方法`
**不是目录真不可写，而是"重启后 0 秒就去读"**；只要等 10 秒就正常。
⚠️ 必须**同时修** `note="已重启确认"` 的语义：它当前只表示"重启过一次"、
**不等于**"结论可信"（上例中该结论恰是错的），而后端熔断计数只认这个词，会据此误判后端死。
详见计划文档 §12.14.4。
