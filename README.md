...
**真瓶颈另在「目录可写性预检」**：`_fix_probe_dir_writable` 的写探针走**与 mkdir 同一条
会回 409 的写路径**，于是「建目录失败」必然导致「探针写不进 → 判目录不可写 → 跳过全部
4 种修复方法」，同一个故障被计费两次（详见计划文档 §12.14）。
...
修法方向因此不是"换驱动/换建目录通路"，而是**让兜底目录跳到故障子树之外** ——
现有短哈希兜底目录正好也落在同一棵坏子树里，故同样 409，这正是生产里
「换成短哈希目录 → 同样 409 → 兜底终止」的直接原因。

🔑 **★★ 最高优先级：把「目录建不出」与「目录不可写」解耦（2026-09-18 实测，run `35296822507`）**：
探针写失败**只是复现了 mkdir 的 409 写路径**（§12.14.1 已证），**不代表目录不可写**；
而 `_fix_probe_dir_writable` 失败后调用方直接判"原目录不可写 → 跳过 4 种方法"
（`file_fix.sh:1484-1489`），这是当前最大的误判源。

同轮探针还测到 `重启后写入: +0s=FAIL() +10s=OK +30s=OK +60s=OK`、
`列表可见性: 首次非空=10s` ⇒ 后端重启后约 10 秒才就绪。
⚠️ **但这不足以解释生产失败**：生产路径已盲等 **70s**（驱动就绪 60s + 路径刷新后 10s），
等满后探针**仍然消失**（当日 `copyto` 就是 409）⇒ **主因仍是 409 写路径**。
该时序项的**唯一价值是提醒不要把 `OPENLIST_DRIVER_READY_WAIT` 调短**。

⚠️ 另需修 `note="已重启确认"` 的语义：它只表示"重启过一次"、**不等于**"结论可信"
（若"重启后列表未就绪"造成读失败，会把"读不到"误标成"确认不可写"），
而后端熔断计数只认这个词，存在误判后端死的风险。详见计划文档 §12.14.4。
...
出口基准 22.71 MiB/s（瓶颈确在 wopan 侧）。唯一待查新线索: 字符集阶梯里
**最贴近生产真实文件名的"混合"档 FAIL(405)**（名长不是主因，需单独诊断）。
...
除常规 13 组探针外，`openlist-diag.yml` 还挂了 5 个专项开关，各自独立报告文件：
