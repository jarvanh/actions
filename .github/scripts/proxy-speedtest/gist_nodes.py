...
`yaml.safe_load` 也是 12635），但从开跑到 mihomo 配好只有 1.67 秒，根本来不及给 12635 个
节点逐一出结论，于是下游读到的是一个「才刚探了几个」的快照。**瓶颈是规模。**
在这里先筛一遍，下游拿到的就是几百个活节点，它自己的健康检查能在秒级完成。

⚠️ 同一轮里那行 `source_mapping_built entries: 14` 曾被当成「下游只认到 14 个节点」的证据，
2026-09-26 更正：它统计的是**订阅正文里解析出多少条可导出配置**，低值是
`speedtest_gitee.build_source_mapping` 的 YAML 分支被一行看似链接的节点名整段跳过所致
（见该函数注释与 docs/proxy-speedtest-gistnodes.md 的「为什么 source_mapping 会漏掉整份
YAML 订阅」），与规模无关、也不等于下游拿到的节点数。
