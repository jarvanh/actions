...
# ---------- 磁盘空间预检 ----------
# 备份/恢复历史上涉及 30GB 级 tarball（现主链路已改流式、不落本地盘，见 emby.yml；
# 本预检对兜底/历史路径仍有效），落盘前先确认空间，避免解压到一半才失败、
...
# ---------- 归档临时文件清理 ----------
# 清理备份 tarball（当前主链路已改流式、不落本地盘，见 emby.yml；此清理针对
# 兜底/历史路径的遗留产物）——体积巨大，成功失败都要清掉，否则占满根分区。
