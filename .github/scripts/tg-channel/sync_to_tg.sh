...
def esc(s) -> str:
    ...
    （规范 · 助手速查：内嵌段与共享层同义实现、三处定义保持一致）。此前本函数用
...
def fmt_secs(x: float) -> str:
    """时长中文形态（规范 · 时长写法 时长五层，按量级选，勿自创形态）。
...
def tg_entry(subject, *meta, code: bool = True):
    ...
    本文件是内嵌 python 段、无法 import 共享层，故在此同义实现（保持三者一致）：
    条目主体 + 元数据统一 " · " 分隔、统一经 esc() 转义、顺序固定（规范 · 条目与树形）。
...
def build_fail_notify(title: str, file: str, elapsed: float, lines: list):
    ...
    """
    parts = [
        # 标题 = emoji + 短语（裸文本，规范 · 取值行口径；此前加粗与 tg_add_title 版式漂移）
        # 标题（emoji + 短语）裸文本；值/计数按 规范 · 取值行口径
        f"{esc(title)}",
        TG_SEP,
        # 文件名属机器值 → <code>；emoji 不套标签（规范 · 取值行口径）
        f"📁 {tg_entry(shorten_name(os.path.basename(file)))}",
        f"📦 分组：{tg_entry(CAPTION_PREFIX)}",  # 机器值 → <code>（规范 · 取值行口径）
...
def get_video_list():
    ...
    if result.returncode != 0:
        ...
        notify("\n".join([
            ...
            # 多行日志走 <pre>（与下载失败通知同款口径：规范 · 失败与异常要求给出原始输出）
...
# 列表整行只转义不加标签，与同通知内的跳过明细两种条目风格并存（规范 · 条目与树形）
_render_named_entries() {
  ...
  [ -z "$_out" ] && return 0
  # 超 8 条折叠（规范 · 折叠规则）：条目已由 tg_add_entry 构建（已转义且含 <code>），
...
_render_skipped_groups() {
  ...
  while IFS= read -r _g; do
    ...
    # 超 max 条折叠交给 tree_fold（规范 · 折叠规则）：它只截断 + 加折叠行，不再转义。
...
msg=""
# 状态图标随结论降级（规范 · 状态图标）：本次全部失败 → ❌；部分失败 → ⚠️；否则 📺
