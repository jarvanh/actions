...
#      file_fix_pipeline.sh 持久化验证（_persist_verify_entries，重启容器后逐文件复核）
#      发现假成功后，本轮立即换方法重试（黑名单让 try_fix_failed_file
#      直接跳过失效方法）；跨轮修复同样跳过，避免方法1 每轮都白白"成功"
#      一次再被发现。
#
# 依赖: utils.sh (log_fix), openlist_api.sh (_get_openlist_token), telegram.sh (间接),
...
# 由 file_fix_pipeline.sh 修复管线初始化 _RAW_VERIFY_* 全局后启用（未初始化 = 未启用，
...
#   成功可能漏检（计数随缓存条目一起增长）——权威兜底是 file_fix_pipeline.sh 持久化
...
try_fix_failed_file() {
  ...
  if ! _fix_probe_dir_writable "$actual_dst_dir" "$actual_ol_dir"; then
    ...
    if ! _fix_switch_to_hash_dir; then
      ...
      # 原因里必须带上"哪一步没成立"：熔断未探测、备用目录也不可写是两件不同的事，
      # 旧文案一律归为"均未通过可写性预检"（2026-09-12 修）。
      # 注: "无目录可换"（根目录文件直接放弃换目录）这一支已于 2026-09-16 放开 ——
      # 根目录文件现在与子目录文件同口径（也会真去建+探备用目录）
  ...
  #
  # **根目录文件同样要走这一步**（2026-09-16 修，与 `_fix_switch_to_hash_dir` 内那道闸
  # 是同一次改动的两半）: 此处原先还有 `file_dir_rel != "." && -n` 两个条件 ——
  # 与内层闸**重复**，而且这里是**静默跳过**（连日志都不打）⇒ 根目录文件在
  # "4 方法全败"之后根本不会尝试换路径，日志里只留一句聚合文案
  # 「❌ 全部修复方法（1-4）均失败（含短哈希目录兜底）」—— **文案与事实相反**。
  # 实测 run 34940234180 的 5 个顽固文件（均在子任务根目录、`.mp4`、200–250MB）正是
  # 如此: 全日志既无「目录级兜底」也无「无目录可换」，从未试过"换路径"这一维。
  if [ "$used_hash_dir" -eq 0 ] && [ "${OPENLIST_HASH_DIR_FALLBACK:-1}" != "0" ]; then
