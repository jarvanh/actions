...
#   - 改名类（短哈希文件名、base64URL 编码目录变体、短哈希目录变体）: rclone move
#     服务端移动回原路径（不经过本地，不重新上传）
#     短哈希目录是不可逆的（整条目录折叠成 8 位 md5），原目录名只能取 marker
#     的 original 字段——本文件全程按 alternative/original 完整路径移动，
#     不解析目录名，故无需为它单独分支
...
# 条目列表渲染: 每组上限 8 条，超出折叠"还有 N 条…"（规范 2.2 节/第 4 章，
