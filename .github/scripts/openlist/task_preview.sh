...
#   不按总数盲减（历史用总量差值法时产生过 "+6.7 GiB / +0 文件" 的矛盾展示，
#   盲减还会把真实缺失文件从预估中吞掉）。
#   目标端清单获取失败时（驱动懒加载/网盘限流等）: 快速失败先重试，
...
# 返回: "bytes count"
# 注意: 该函数对大源（925GiB 级）单次全量递归列举要 7-9min —— **仅注册模式
# （TASK_REGISTER_ONLY=1）不得调用它**，注册阶段的"源端 X GB"只是面板展示，
# 调用点（_preview_register）已按模式跳过；预览模式（skip_preview=false）才需要，
# 且走 add_preview_pair 先行填充的 PREVIEW_SRC_LIST_CACHE，为命中缓存。
...
_preview_render_pairs_detail() {
  ...
  while IFS=$'\t' read -r _task _src _excl _sbytes _scount _dst _ybytes _ycount _ynew _yupd _fnote _dfail _pskip; do
    ...
    # 规范 · 发送层是硬约束：动态内容必须转义，没有「值是脚本自造、字符集受限」的豁免条款
  ...
  # 组头是分节标题 → 裸文本（规范 · 分节），条目内的路径才走 <code> 等宽
  local _out="" _src _gi=0
  for _src in "${_g_order[@]}"; do
    [ "$_gi" -gt 0 ] && _out+=$'\n'
    _out+="📁 $(escape_html "$_src")"
...
flush_task_preview() {
  ...
  for _t in "${_tasks[@]}"; do
    ...
    if [ "${PREVIEW_TOTAL_UPD_COUNT:-0}" -gt 0 ]; then
      # 计数细节一律 " · " 分隔并入合计行（旧「（新增 X · 同名更新 Y）」形态废弃）
    ...
    if [ "$_skb" -gt 0 ] || [ "$_skc" -gt 0 ]; then
      ...
      _skip_note=$'\n'"⏭️ 本轮预计跳过：$(format_bytes "$_skb") / ${_skc} 文件 · 预计实际传输 $(format_bytes "$_real_b") / ${_real_c} 文件"
    fi
