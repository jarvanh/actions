...
#            └─ ❌#47 ✅00 🔧00 ❗22 ⏭️00 ♻️00 ⏱00:58 ⬆️4.72G    批次历史（tree_lines，字段表见 docs 规范 · 计数与量化）
...
_progress_render() {
  ...
  if [ "$running" -gt 0 ]; then
    ...
    # 「未执行完」是说明而不是计数的一部分（规范 · 分节：分节标题 = emoji 分节 · N），
    # 故下沉为说明段——与 规范 · 折叠规则「缺失的目录 · 可能被删除 · N」的收敛方式同款
