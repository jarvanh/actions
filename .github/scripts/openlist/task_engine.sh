...
# 默认 1 → **2**（2026-09-15）: 隔离条件下的跨后端实测证明**两个挂载的额度相互独立**
#   —— 两侧各 12 流: A(wopan176Crypt)=1.68 + B(wopan175)=1.88 = **合计 3.37 MiB/s**，
#   而同轮单后端 12 流只有 1.78（≈1.9×）⇒ 并行吃第二个挂载的额度是净收益。
#   （原"共享瓶颈"结论是在有主轮并行的条件下测得的，属污染样本，已作废；
