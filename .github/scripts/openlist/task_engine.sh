...
#       最终检查。
# 何时不跑（F10，2026-09-19）: 调用方 sync_by_file_batches 在调本函数**之前**
#       先做 405 快速失败判定（_batch_405_flood）—— 该目录已确定性拒写时直接
#       中止剩余批次、跳过本函数（重启 + 全量 lsf + 重试 + 修复对这个目录纯属
#       白烧）。故日志里本批无巩固输出不等于巩固坏了。
