...
# 背景: 进度面板 progress_task_done 分类与 run_all_tasks 轮转游标都只认 SYNC_FAILED/
#   SYNC_SKIPPED 全局标志；实现层若只回传非零返回码而漏置标志（批次预检
#   熔断分支的历史缺陷形态），失败任务会被双双记成"已完成"。
# 契约:
#   M1 impl 返回 1 且未置任何标志 -> 兜底 SYNC_FAILED=1 -> progress_task_done("failed"),
#      sync_task 透传返回码 1
#   M2 impl 返回 0 且标志全零 -> progress_task_done("completed")，不受兜底影响
#   M3 SKIP 任务（impl rc=1 + SYNC_SKIPPED=1）-> progress_task_done("skipped")，
...
BEGIN_CALLED=0
progress_task_begin() { BEGIN_CALLED=$((BEGIN_CALLED + 1)); }
DONE_STATUS=""
progress_task_done() { DONE_STATUS="$1"; }
FIX_SAVED=0
...
chk "M1 progress_task_done 判 failed" "$DONE_STATUS" "failed"
...
chk "M2 progress_task_done 判 completed" "$DONE_STATUS" "completed"
...
chk "M3 progress_task_done 判 skipped" "$DONE_STATUS" "skipped"
...
chk "M4 progress_task_done 判 failed" "$DONE_STATUS" "failed"
