...
#
# ⚠️ 还原语义（2026-09-26 用户拍板）: 改名类用 **copyto 保留替代副本**（原为 moveto
#   搬走即删）。故断言方向为"原路径已复原 **且替代副本仍在**"——副本是短哈希体系下
#   唯一的内容载体（哈希不可逆），删了这条记录就再无法自证。1c/2c/8f 锁的就是
#   "副本必保留"；若有人退回 moveto，这三条立刻转红。
#
...
[ -f "$DEST/$ALT1" ] && ok "1c 替代短名文件仍保留（备份副本，未搬走）" || bad "1c 替代短名文件被搬走（副本丢失）"
...
[ -f "$DEST/$ALT2" ] && ok "2c 折叠目录内替代文件仍保留（备份副本）" || bad "2c 折叠目录内替代文件被搬走"
...
[ -f "$DEST/$ALT8" ] \
  && ok "8f 还原保留备份副本（替代位置仍在，可追溯）" || bad "8f 替代文件被搬走（副本丢失）"
