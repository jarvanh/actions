...
# T14/T15（2026-09-15 加）: **折叠落盘延迟复核** —— 列表可见性延迟远超分钟级
#   （判"零落盘"的目录 11h 后才读到文件），故"读空"不再当场判失败，而是登记后
#   在处理完全部目录时统一复核一次：可见即补记 marker 并把文件从缺失清单移除，
#   仍不可见才真正退回逐文件修复（旧行为是双重损失: 逐文件打原目录=405 那条路径
#   + 成果不进 marker ⇒ 幽灵落盘，下轮再折一次）。
set -u
