...
chk "L3a 批次标签挂树形连接符（code 等宽）" \
  "$(line_of '文件批次拆分')" "<code>          </code><code>  └─ </code>${BATCH_LABEL#▸ }"
...
chk "L4a 树行是最后一条（🔄 已置尾，深层块才挂得住）" \
  "$(printf '%s' "$ACT_ROW" | grep -c '</code>🔄')" "1"
...
chk "L5b 深层 detail 不污染任务行（任务行无 detail）" \
  "$(line_of 'wopan176Crypt/0')" "<code>  └─ </code><code>wopan176Crypt/0</code>"
...
chk "L7c 目标端行无 detail 黏连" \
  "$(line_of 'wopan175/0')" "<code>  └─ </code><code>wopan175/0/j-1024j</code>"
