...
#   10g/10h. **判死前置条件**（2026-09-15 加，run 34920298417 实锤）:
#      本批 copy 被预算硬上限 timeout 截断（rc=124）或可用工作片 ≤ OPENLIST_F5_MIN_SLICE
#      ⇒ **不判**后端全拒（"未落盘"是预算造成的，不是后端拒收）——否则会经 F6 把
#      健康后端写进 backend_dead.json、让它在 TTL 内被整轮跳过。
