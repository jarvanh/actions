...
#      是 openlist_driver.sh _refresh_ol_drivers 的驱动刷新三招（驱动刷新方法1 load_all /
...
# openlist_driver.sh _refresh_ol_drivers 的三招是另一套"方法N"，领域限定词必须不同
for _d in "$D1" "$D2" "$D3" "$D4"; do
  case "$_d" in *"驱动刷新"*) bad "3 文件修复全名误含'驱动刷新'限定词: $_d" ;; esac
done
ok "3a 文件修复全名不含'驱动刷新'限定词（领域不串）"
# 反向: openlist_driver.sh 里"驱动刷新方法N"的运行时日志不得带上文件修复限定词
# （注释里提到对方领域是正常的——那正是消歧说明；只查实际输出的日志行）
if grep -n 'echo .*驱动刷新方法' "$_REPO_ROOT/.github/scripts/openlist/openlist_driver.sh" | grep -q "文件修复"; then
