...
#   3. 源端清单失败 → 无变动（不误报全量）
#   4. 目标端清单失败 → 快速失败重试至多 3 次后按全量待同步估算，
#      且预览明示 ⚠️（条目子行 + 合计附注——静默全量会把目标端已有大半
#      文件的对虚报成全量，合计虚高数倍）
#      注: "超时型失败不重试" 的判据为「单次尝试耗时 ≥ 预览 listing 超时」
#      （F17，2026-09-19 由固定 300s 改；超时默认 240s）—— mock 的 sleep 为空，
#      测试走的是"错误型失败"，故仍能看到 3 次重试
#   5. 无 marker 修复记录 → 缺失文件全额计入
#   6. 渲染: 差异构成子行 / 无变动 / 合计构成附注
#   F17 预览短超时: _get_listing_json 用 OPENLIST_PREVIEW_LISTING_TIMEOUT（默认
#      240s）而非 sync 侧的 OPENLIST_RCLONE_LISTING_TIMEOUT（900s）；mock 的
#      timeout 为直通，故此处只锁"取的是预览专属变量"这一结构，超时值本身由
#      workflow 注入 + 静态检查覆盖
set -u
PASS=0; FAIL=0
...
echo "$SEND_CAPTURE" | grep -q '排除：<code>notion/\*\*</code>' \
  && ok "1i3 单条排除并入子行（不扩树，规范 · 条目与树形）" || bad "1i3: $SEND_CAPTURE"
...
_reset_preview_state "srcfail" >/dev/null
...
_reset_preview_state "dstfail" >/dev/null
...
MARKER_JSON='{}'
_reset_preview_state "nomarker" >/dev/null
...
DST_JSON='[]'
MARKER_JSON='{}'
_reset_preview_state "purenew" >/dev/null
...
DST_JSON='[]'
MARKER_JSON='{}'
_reset_preview_state "biglist" >/dev/null
...
MARKER_JSON='{}'
_reset_preview_state "flaky" >/dev/null
