...
_send_sync_result_notification() {
  ...
  if [ -s "$fix_list" ]; then
    ...
    # 超 8 条折叠交给真源 tree_fold（规范 · 折叠规则）：条目流已由 tg_entry* 构建（已转义、
  ...
  if [ -s "$fail_list" ]; then
    ...
    # （规范 · 状态图标：图标要与结论一致），且空分节白占版面（2026-09-12 修正）
    ...
    if [ "$is_partial_failure" -eq 1 ]; then
      ...
      # 英文 token 中文化（规范 · 失败与异常：退出码 45，而不是 exit=45）
    ...
    if [ "$err_log_size" -gt 0 ] && [ "$err_log_size" -lt "${OPENLIST_ERR_LOG_MAX_BYTES:-50000000}" ]; then
      ...
      # 重试次数与发送层对齐（5 次；此前 3 次，与 规范 · 发送层「429 重试最多 5 次」口径不一）
