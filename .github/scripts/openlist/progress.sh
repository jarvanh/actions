...
#   PROGRESS_TASKS_FILE      — 任务队列（TSV: task_id, display_name, status, detail）
...
_progress_set_task_status() {
  ...
  while IFS=$'\t' read -r tid tname tstatus tdetail; do
    if [ "$tid" = "$task_id" ]; then
      echo -e "${tid}\t${tname}\t${status}\t${detail}" >> "$tmp"
    else
      echo -e "${tid}\t${tname}\t${tstatus}\t${tdetail}" >> "$tmp"
    fi
  ...
  if [ -f "$PROGRESS_TASKS_FILE" ]; then
    while IFS=$'\t' read -r tid tname tstatus tdetail; do
      ...
      case "$tstatus" in
        ...
        running)
          running=$((running + 1))
          running_list+="${display}"$'\n'
          ;;
        completed)
          completed=$((completed + 1))
          completed_list+="${display}"$'\n'
          ;;
        skipped)
          skipped=$((skipped + 1))
          skipped_list+="${display}"$'\n'
          ;;
        failed)
          failed=$((failed + 1))
          failed_list+="${display}"$'\n'
...
# 标记任务开始（自动注册未注册的任务）
# 用法: task_begin <task_id> [detail]
task_begin() {
  local task_id="$1"
  local detail="${2:-}"
  # 自动注册未注册的任务
  if ! grep -qP "^\Q${task_id}\E\t" "$PROGRESS_TASKS_FILE" 2>/dev/null; then
    progress_register_task "$task_id" "$detail"
  fi
  echo "$task_id" > "$PROGRESS_CURRENT_FILE"
  _progress_set_task_status "$task_id" "running" "$detail"
  _progress_refresh
}
