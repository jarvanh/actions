...
def restore_info($orig; $alt; $method; $src; $dst):
  ...
  #   "rclone copyto（短哈希目录 <hash> + 原文件名）"          → hash_dir (目录级兜底变体)
  #   "rclone copyto（短哈希目录 <hash> + 短哈希文件名 <hash>）" → hash_dir（目录与文件名双改）
  ...
  # 目录级兜底（file_fix.sh _fix_escape_to_hash_dir）: 排在短哈希文件名之前——
  #   两者可同时出现（短哈希目录 + 短哈希文件名），目录变更是更结构性的差异，
  #   归 hash_dir 并在 summary/steps 里一并说明文件名状态
  | ($method | test("短哈希目录 ")) as $has_hash_dir
  ...
  | if   $has_split_zip then {kind:"split_zip",
    ...
    elif $has_hash_dir then {kind:"hash_dir",
      # 对象值不能裸用 + 拼接（{a: "x" + "y"} 非法），必须括号包裹——见文件头注释
      summary: ("整条目录路径折叠为 8 位 md5 短哈希目录 · 规避目录名过长/敏感词/加密路径超限" + (if $has_short_hash then " · 文件名也替换为短哈希名" else " · 文件名保持原样" end)),
