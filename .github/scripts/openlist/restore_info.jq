...
def restore_info($orig; $alt; $method; $src; $dst):
  # 现行 4 种修复方式精确识别（文案来源: file_fix.sh _succeed 各调用点）
  # ⚠️ 2026-09-16 命名统一后，方法描述形如 `方法3·分卷·原名（…）`；
  #   下面的 test() 子串**必须与 file_fix.sh _fix_method_desc 保持同步** ——
  #   丢了子串本分类程序会静默退化成 kind=copy（还原脚本错误）。
