#!/bin/bash
# 重置 Telegram 频道共享脚本（清空所有消息并删除 uploaded_videos.txt）
#
# 流程：
#   1. 调用 clean_tg_channel.py 删除频道内所有消息
#   2. 删除远端 uploaded_videos.txt，使下次运行重新处理全部视频
#   3. 发送 Telegram 通知
#
...
#
# 环境变量:
#   SOURCE_REMOTE          - rclone 远程路径（用于删除 uploaded_videos.txt）
