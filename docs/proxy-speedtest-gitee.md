...
这正是 taier 侧 `TAIER_ALIVE_PROBE`（默认开）存在的意义——它按节点逐个探测，判死只认 mihomo 的
明确结论、机制出错一律 fail-open。若要限制总量用 `PROXY_SPEEDTEST_MAX_NODES`。

## 为什么开测前要等 provider 展开（三套共用）

`wait_provider_ready(names)` 在**逐节点循环之前**跑：拿头尾各两个节点名探
...
**封顶 300 秒**），**不得写死 60**。为什么：展开耗时随节点数增长，写死 60 秒在大池子上必然
等不完——2026-09-22 编排轮 2123 个节点等满 60 秒（`attempts=138` 全 404）仍未展开，
taier 开测后头 8 个探测全 `Resource not found` 触发熔断关掉测活 ⇒ **1498 个死节点没被提前
筛掉、全跑满 15.9 秒的测速超时**（1498 × 15.9s ≈ 6.6h，单这一项就吃掉整个 5 小时预算）。

⚠️ **参数是加载总量，不是「过滤后要测的候选数」**（2026-09-23 二次修正，踩过一次）。
展开耗时取决于 mihomo **装了多少**，与调用方之后砍到多少**无关**。第一版按候选数算：
...
⚠️ **封顶 300 秒，不要再往上加**（2026-09-23 从 900 下调）。判据是日志里的**成功事件**：
`provider_ready` 在**所有**历史轮次中**从未出现过**，而 `provider_ready_timeout` 每轮必有——
包括给到 900 秒那轮：`waited=900.34 attempts=1810` 全 404。加时换不来「等到」，只换来了
「白烧预算」（15 分钟纯空转）。而降级路径本身是安全的：taier 关掉测活、全量放行去测速；
