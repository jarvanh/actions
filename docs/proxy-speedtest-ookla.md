...
4. 只在**目标候选池内部**顺延，两种情况：编号失效（NoServersException，全局剔除）、
   ...
   「按节点出口就近」——那等于换个口径把难看的数据盖掉；
   实测（2026-09-11，日本出口节点）：同一批候选里 24447 / 16204 连不上、30852 通
   （285ms、↓0.48 ↑1.11 MiB/s），固定死一个点会整轮空跑，所以顺延必须存在；
   连续 `OOKLA_ABORT_AFTER` 个节点都整体兜底时，才认为这批节点对目标地区整体不通，
   后续节点直接走就近（省得每个节点都白试一遍）；
5. 候选全失效 / 目标区域无点 → 退回「按节点出口就近」，通知里用
   `<目标点> → 按节点出口就近 · <原因>` 明确标注，**绝不静默换口径**；
6. 实际用的点距目标 ≥200km 时（默认锚点广州必然命中），通知里标注
   「距目标 xxxkm · 目标附近无点，已扩大检索」。

**口径决策（2026-09-11 定）：大陆优先，接受低成功率。** 实测 8 个境外节点只有 1 个能测通
大陆点（昆山 30852，270ms ↑8.23 ↓3.49 兆）、其余全部不可达。这是真实结论而非故障，
与同族（gitee / cdn / taier 都测「节点 → 大陆」）保持同口径更重要，因此**不**为了出数据
改回「按节点出口就近」。想看节点自身带宽能力请用另三套。
...
| 同一批候选里有的点通、有的不通 | 正常现象：大陆测速点对境外出口的可达性差异极大（实测日本出口：24447/16204 不通、30852 通）。脚本会顺延到下一个候选，`ookla_point_unreachable` 日志记录每个点的失败次数 |
| 整轮几乎全失败（`Cannot read` / `Latency test failed`） | 所有目标候选对这批节点都不可达 → 连续 `OOKLA_ABORT_AFTER` 个节点后转全局就近兜底。日志 `ookla_server_listed count=0` + 控制面 403/429 时，说明 speedtest.net 对该节点出口风控（已知风险）；想拿可用数据就放宽 `OOKLA_TARGET_CC`、换锚点，或用 taier/gitee 口径交叉验证 |
| `-L` 列表拉不到（count=0）但 `--server-id` 能测通 | 正常：`-L` 走 `api.speedtest.net`（机房 IP 常被 403/429），`--server-id` 直连测速点主机不走控制面。**这也是本工作流必须锁点、不能靠就近列表的原因** |
...
- **TUN 对 UDP 可能不命中 `PROCESS-NAME` 规则**：诊断日志里能同时看到
  `[TCP] ...(speedtest) match ProcessName(speedtest) using AUTO` 与
  `[UDP] ... match Match using DIRECT`（mihomo 对 TUN 的 UDP 连接有时查不到进程）。
  实际带宽走 TCP、受规则保护，但**UDP 相关的延迟探测可能未走代理**，延迟值需谨慎解读。
