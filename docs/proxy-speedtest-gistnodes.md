...
⚠️ **词表不是一劳永逸的：池子每轮都在变大。** 跨轮累积只增不减，实测两轮之间
发布节点 16012 → 19369、累积文件 5.46MB → 6.57MB，同一个词表的 kept 从 1647 涨到 2123
（≈6.7h，重新超出预算）。所以**每轮都要按下面的方法重算**，别把某一次的统计当常量。

现词表 `IPLC|IPEL|IEPL|HK|Hong|港|SG`（2026-09-22 第二次收紧），等于优先级正则去掉
「专线 / TW / 台 / Taiwan / 新加坡」。依据是拉当轮真实发布池逐词统计**独占命中**
（去掉该词后少掉多少命中），本轮 19725 节点上的读数：

| 词 | 独占命中 | 处置 |
|---|---|---|
| `专线` | 757（首轮统计） | 去掉：**几乎全是游戏加速线**（无畏契约/王者荣耀/PUBG→日/英/法/澳/瑞士等非优质地区），既非优质地区又最占窗口 |
| `TW` / `台` / `Taiwan` | 131 / 214 / 0 | 去掉：合计让出 353（台湾地区），本轮为压进预算让出 |
| `新加坡` | 295 | 去掉：`SG`（422）已覆盖英文侧，中文词是冗余覆盖 |
| `HK` / `港` / `Hong` | 471 / 377 / 219 | **保留**：香港共 1093，占命中池最大比例 |
| `SG` | 422 | 保留（新加坡英文侧） |
| `IPLC` / `IPEL` / `IEPL` | 0 / 0 / 6 | 保留作容错（命中全被别的词覆盖，留着不花钱） |

**调整方法**：拉最新发布池（`proxy_speedtest_gistnodes_providers.yaml` 的 raw URL），逐个
词算「去掉它 kept 掉多少」，按独占命中**从小到大**砍，直到 `kept × 单节点耗时 < 预算`。
单节点耗时按当轮实测算（`taier_speedtest_done` 的 `node_count` 与运行时长相除，本轮
≈11.4 秒）；目标 kept ≤ 1800，收紧一档取 ~1540（1544 × 11.4s ≈ 4.9h，给收尾留余量）。
