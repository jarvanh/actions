...
**调度为什么是每天 2 次、京 08/16。** 原先 4 次（京 08/12/16/20），2026-09-17 减半：
单轮要跑 20–55 分钟，4 次/天时后一轮常与前一轮尾巴重叠、又频繁被手动触发的新轮顶掉
（实测当天连续 7 轮 cancelled/failure，结果文件因此长时间不刷新），减到 2 次让每轮有
充足窗口跑完。这两个点刻意避开三套测速的定时轮（gitee 京 05/09/13/17、cdn 06/10/14/18、
taier 07/11/15/19）：本工作流抓完节点会**直接调用**其中一套来测速，撞点会互相抢节点带宽、
读数彼此污染；08/16 各在 taier（07/15）之后 1 小时，与所有测速点至少错开 1 小时。
仍只排在北京 05:00–20:00：夜间 runner 排队 + 出口拥塞会让测速读数失真。
...
[taier 文档 · include 过滤](proxy-speedtest-taier.md#include-过滤唯一决定池子有多大的一层2026-09-22)）。
**为什么必须过滤**：这是**唯一**能把池子压小的一层——2026-09-22 run 35684842162 实测交接
15793 个节点，5 小时预算（18000 秒）只测完 1905 个（单节点 ≈9.5 秒，全测完 ≈41 小时），
轮轮撞预算到点收摊。（旧的优先级排序只排顺序、不减量，`max_nodes` 是按原序砍尾巴会误伤
未测节点，两者已于 2026-09-24 一并删除。）

⚠️ **池子会波动，但决定成本的是「存活率」不是「kept」**（2026-09-25 重写模型）：
跨轮累积让池子长期只增不减，但单轮抓取量波动极大——实测 before 从 24575 到 3726（6.6 倍）。
命中率却稳定在 7.5–8.5%，所以 **kept 随池子同比例波动**，拿单轮的 kept 当常量没有意义。

成本模型（关键，推翻了旧结论）：

    整轮测速耗时 ≈ kept × 存活率 × 42 秒

**duration 涨价的代价只落在活节点上**。测活改成整组延迟表后，死节点是毫秒级查表跳过，
不再吃满测速窗口——run 36108365084 实测 kept=318、`probe_alive=13`/`probe_dead=305`，
整轮 9m54s 里测速段 ≈554s ≈ 13 × 42s，305 个死节点几乎不占时间。所以约束不是
「kept × 42 < 预算」，而是「kept × 存活率 × 42 < 18000」：

| 存活率 | worst case（池 24575、kept≈2097）耗时 | 判断 |
|---|---|---|
| 4%（实测） | ~84 个活节点 × 42s ≈ **1.0h** | 远在 5h 内 |
| 20% | ≈ 4.9h | 勉强够 |
| >25% | > 5h | 超预算 |

⇒ **维持现词表**。按 worst case 收紧到 kept≈800 需要砍掉 `HK` 或 `SG` 其中一个地区词
（覆盖减半），换来的余量在实测存活率下根本用不到，是净损失。

现词表 `IPLC|IPEL|IEPL|HK|Hong|港|SG`（2026-09-22 定，2026-09-25 复核保留），等于原优先级
正则去掉「专线 / TW / 台 / Taiwan / 新加坡」。2026-09-25 在当轮真实池（3726 个）上的读数：
kept=318（命中率 8.5%），各词**独占命中**（去掉该词后少掉多少命中）：

| 词 | 独占命中 | 处置 |
|---|---|---|
| `SG` | 117 | **保留**（新加坡英文侧，独占最多） |
| `HK` | 77 | **保留**（香港英文侧） |
| `Hong` | 67 | **保留**（香港另一拼法） |
| `港` | 34 | **保留**（香港中文侧） |
| `IEPL` | 4 | 保留作容错 |
| `IPLC` / `IPEL` | 0 / 0 | 保留作容错（命中全被别的词覆盖，留着不花钱） |
| `专线` | — | 去掉：**几乎全是游戏加速线**（无畏契约/王者荣耀/PUBG→日/英/法/澳/瑞士等非优质地区），既非优质地区又最占窗口 |
| `TW` / `台` / `Taiwan` / `新加坡` | — | 去掉：为压进预算让出的地区；`新加坡` 已被 `SG` 覆盖英文侧 |

**⚠️ 触发重算的条件**（满足任一才动，别照抄旧结论）：
1. 日志里 `taier_speedtest_done` 的 `probe_alive / (probe_alive + probe_dead)` **存活率 > 15%**；
2. 编排轮整轮耗时 **> 4.5 小时**，或出现 `taier_budget_stop`（到点收摊）。

**重算方法**：拉最新发布池（`proxy_speedtest_gistnodes_providers.yaml` 的 raw URL），逐词算
「去掉它 kept 掉多少」，按独占命中**从小到大**砍，直到 `kept × 存活率 × 42 < 18000`。
命中判据 `re.IGNORECASE` + 只看节点名，与下游 `filter_nodes_include` 同口径；下游有两条
fail-open（正则非法 / 过滤后为空 → 原样放行），词表写错不会让下游零产出，但会退化回
「不过滤全测」——所以改完看一眼 `nodes_include_filtered` 的 `kept` 是否仍在目标内。

另一个收口方向（未采用，留作备选）：给编排轮的 `test_nodes` 设固定上限（如 1400）。
（2026-09-24 起下游 taier 已不再接 `max_nodes` 入参——截断是「按原序砍尾巴」、会砍掉还没测过
的节点，与「到点收摊」重复且更易误伤。若真要兜底，得在**本层**发布时就限量，而不是塞给下游。）

**只对 taier 引擎生效**：gitee / cdn 未实现 include 过滤，`PROXY_SPEEDTEST_ENGINE` 切到
它们时编排轮仍是不限量全测（它们的预算同为 18000 秒，照样到点收摊）。

观测：下游日志 `nodes_include_filtered`（`before`/`kept`/`dropped`，过滤后池子）→
`nodes_collected`（最终进循环数）。
（旧的 `nodes_prioritized` 已随 2026-09-24 删除的优先级排序一并消失，别再拿它当判据。）
...
| 编排轮测速怎么还是测不完 / 想核对过滤有没有生效 | 下游 taier 日志按序看 `nodes_include_filtered`（`before`/`kept`/`dropped`，include 过滤只对 taier 引擎存在，**它是唯一决定池子大小的一层**）→ `taier_budget_stop`（`tested`/`total`）。`include_regex_all_dropped_fallback` = 词表与当轮节点命名完全错位、已退化为不过滤全测（fail-open，不会零产出）；`include_regex_invalid` = 正则语法错。⚠️ 2026-09-24 起 `nodes_prioritized`（优先级排序，只排顺序不减量）已随该功能删除，日志里不应再出现。词表依据与调整方法见「编排轮 include 过滤词表」 |
| 整轮挂在 `Sub-Store 产出的 YAML 解析失败：unacceptable character #x…` | 抓取来的**节点名**里混进了 C1 控制字符（实测 `U+009F`，run 36063768399 / 36064746041 连续两轮同位置 8.09MB）。Sub-Store 不校验、原样写进产出，而 `yaml.safe_load` 按规范拒绝 ⇒ 本层 exit 1、下游三个测速 job 全 `skipped`。**且不会自愈**：脏节点在累积文件里，下一轮照样喂进来。现已在解析前 `strip_nonprintable` 清洗（洗的是后面要发布的那份文本，不是临时副本——留着脏字符 mihomo 也解析不了同一份 provider），命中时记 `gist_nodes_yaml_sanitized`（带 `removed` 计数）。`\t` / `\n` / `\r` 是 YAML 合法空白，保留不洗 |
