...
- `concurrency: emby-singleton` —— 同时只允许一个 run，在跑轮不被打断（自续触发正是靠它排成一条链）；
  但**排队中的 run 会被新的 dispatch/cron 顶掉**（concurrency 组内 pending 替换，在跑轮不受影响）——
  想掌控节奏就 `-f self_retrigger=false`，且别在 cron 临近时手动 dispatch。判「run 为什么还没动」
  看 job 的 `startedAt` 而非 run 创建时间；人工取消非即时（2–4min 宽限期）
...
| `EMBY_PREFETCH_PAR_BUSY` | 有真实播放时的回填并发（默认 `2`）——回填是后台优化，让它给播放取链让路 |
| `EMBY_PREFETCH_MAX_DIRS` 的上限语义 | 见下「回填是每轮后台 Graph 工作的硬上限」 |
...
### 4.5 dir_cache 跨 run 持久化（2026-09-14 加）

`dir_cache` 原本是纯进程内 dict，run 一结束就蒸发：每轮启动都从 0 爬，
实测覆盖率只有约 1/3（26,360/83,464 文件），冷解析 1.75s 对命中后 0.29s。
现在它随增量备份跨轮传递：

```
落盘：odlink 每 10 分钟 + 收到 SIGTERM/退出前 → /tmp/odlink-dir-cache.json
上云：emby_incbak.sh 做快照时顺带拷进 STAGE（在 chown 之后，否则 runner 写不进）
恢复：下轮 install emby 从 live 恢复到 /var/lib/emby/odlink-dir-cache.json
装载：**首次解析时**才读（odlink 比 install emby 先启动，那时文件还没恢复）
```

配套的四件事：

- **失效钩子**：解析失败 / 取直链失败立刻删掉该条目并失效同名直链缓存——跨轮
  缓存最大的风险是网盘里改名/移动后仍命中老坐标，宁可多花一次冷解析
- **上限 5 万条**（`ODLINK_DIR_CACHE_MAX`）：超出按插入顺序丢最旧的，防止膨胀
- **严格降级**：文件不存在 / 损坏 / schema 不符 / 条目格式不对 = 现状冷启动，
  落盘失败只记日志。装载路径只读、落盘路径可写，两边都不影响服务可用性
- **落盘前置守卫**（2026-09-14 加）：`dump_dir_cache()` 在 `_dir_loaded` 为假时
  **跳过落盘**，快照侧（`emby_incbak.sh`）再比较一次条目数——新快照少于 STAGE 里
  刚恢复出来的那份就不覆盖。原因：odlink（step 10）比 install emby（step 11）
  先启动，第一次定时落盘（T+600s）必然早于备份恢复（T+~840s），此刻内存里只有
  bootstrap 写进去的 8 个顶层快捷方式；直接写出去会把云端累积的几万条冲掉
  （实测远端每轮开头有约 25 分钟只剩 797 字节 / 8 条，第 2 次快照才自愈；这期间
  轮次被取消 / 短轮收尾就会永久丢缓存）。**不能**改成"落盘前先调
  `_ensure_dir_cache()`"：那会在 T+600 就把 `_dir_loaded` 置真，反而把恢复完成后的
  真装载永久挡掉——`_dir_loaded` 只在**装载结束**时置真

> **回填的实际规模（2026-09-14 全量遍历实测）**：库内条目 83,310 个、去重后 **1,734 个
> 目录**，回填后 `dirs_cached` = **85,812 条 / 19MB**（约 47 条/目录、约 220B/条）。
> `ODLINK_DIR_CACHE_MAX` 默认 **20 万**（一倍余量）——上限若低于实际条目数，落盘会按
> **插入顺序**丢最旧的，而最旧的恰好是最先回填的那批（按 DateCreated 倒序 = 最新入库、
> 最可能被点开的内容），丢错方向。
>
> **回填不会抢播放的 Graph 配额**（2026-09-14 加）：两条让路机制——
> ① **播放让路**：每批前查 Emby Sessions，「有 NowPlayingItem **且** 90 秒内还在上报
> 播放进度」即判定有人在看 → 并发从 `EMBY_PREFETCH_PAR` 降到 `EMBY_PREFETCH_PAR_BUSY`
> （默认 2），批次间隔至少放宽到 12 秒。只看 `NowPlayingItem` 不看到进度上报会把
> 「暂停后扔在那儿的会话」也算成播放中，回填就永远让路、一点也填不上，故两个条件都要。
> ② **限流退避**：每批后读 odlink `/stats` 的 `graph.err`，涨了就翻倍批次间隔（上限 60s），
> 连续无新增错误再逐步收回。
> 正因为有这两条，`EMBY_PREFETCH_MAX_DIRS` 才可以给得比较宽松——它只是「每轮后台
> 工作的天花板」，撞到配额时第一个减速的就是它自己。
>
> **为什么不做成"完全不限"**：真正限制回填规模的不是这个数字，而是**每轮的 Graph 调用量**
> ——回填对每个目录都会重新 fs/list 一次（不是"缓存命中就跳过"，因为列目录本身就是回填
> 手段），全库 ≈ 20 分钟后台 Graph 流量/轮，与播放时的取链共享配额。所以
> `EMBY_PREFETCH_MAX_DIRS` 是「每轮后台 Graph 工作的硬上限」，不是覆盖率目标：库变大时
> 它的保护作用才显现（否则会无限膨胀）；而 `ODLINK_DIR_CACHE_MAX` 只管内存/文件体积，
> 有了「内容不变不落盘 + 快照 `cp -p`」后大文件不再进每轮上传量，故可以给足余量。
>
> **取直链的冷/热成本口径（2026-09-14 实测）**：某条目**第一次**取链必然冷解析
> 0.4~1.3s（路径寻址 1 次 Graph 或逐段下钻），取到后进 `dir_cache`/`link_cache`，
> 之后重播/回看预热都是 **3~6ms**。回填的作用就是把"第一次"也提前付掉（库内全部
> 1,734 个目录都已预列）。排查时注意：**缓存键是去掉 `/onedrive` 前缀后存的**，且
> AV 库条目的 Emby 标题与文件路径完全不同——拿标题字串或带前缀路径去匹配缓存键
> 全是假阴性（2026-09-14 曾因此误判"覆盖率缺口"，正确口径下 12/12 条播放过的路径
> 全部命中）。
...
#### 双通道备份 + 开机后台补回（2026-09-14 晚加）

排除 metadata/library 之后，海报原图改成**独立归档通道**（对齐 openclaw 的
"定时归档 + 最终归档"模式），核心通道保持轻快：

| 通道 | 内容 | 节奏 | 远端 |
|---|---|---|---|
| 核心快照 | data / config / plugins / metadata（不含 library） | 每 25 分钟 + 收尾必做 | `onedrive:backup/emby/live/`（170MB 级，7~20s） |
| 图像归档 | `metadata/library/**`（海报/背景图原图，~1GB） | 运行期 T+60min 后台一次 + 收尾最终一次（剩余预算 <15 分钟自动跳过） | `onedrive:backup/emby/images/`（copy 语义，云端始终最新一份） |
| 轻观测 | playlog.log（截断 512KB）+ odwarm-status.txt（十几行） | 每 10 分钟 | `live/` 下同名覆盖 |

要点：

- 图像归档 `--min-age 2m`（跳过仍在写入的文件）、flock 防止定时与收尾叠罗汉；
  copy 语义只传增量——首轮全量约 1GB，之后每轮只有新增海报（秒级到分钟级）
- **开机顺序**：核心恢复（约 3 分钟，Emby 起来、可看片）→ 后台把缺的原图补回
  `metadata/library`（`rclone copy --ignore-existing` 只补缺、不覆盖本轮 Emby
  新写的，约 10~17 分钟）→ 之后首访海报用本地原图缩放，不再现从微软图库下载
- 图像归档失败/跳过都只告警：原图永远可以由 Emby 重新生成，优先级低于一切
- 轻观测与核心快照写同一路径（`live/playlog.log`、`live/odwarm-status.txt`），
  读取入口不变；playlog 截断到 512KB，文件尺寸恒定
...
| 海报墙/首屏预热恒为 0、宽度档显示"无" | 归档 `warmup.log` 的「海报墙/首屏预热宽度档」行 → `wallwarm.log` → `ge2o.log` | **本轮没有客户端浏览时属预期行为**（只预热实际消费过的尺寸）。判据：看 `ge2o.log` 里有没有**非 127.0.0.1** 的 `Images/Primary?maxWidth=` 请求——有却没预热 = 统计链路坏了；完全没有 = 这轮根本没人访问，去看 run 时段对不对 |
...
### 起播 8 秒的归因闭环（2026-09-14 实测，计划 Phase 4）

一次真实播放（2022 老片、6.3GB、1080p h264，客户端出口在美国西岸代理），同次实测：

| 段 | 值 | 性质 |
|---|---|---|
| `[起播] 客户端出画` | **+8s** | 客户端侧唯一量化读数（Sessions `PositionTicks` 首次 >0） |
| 你 → Cloudflare sea10 → Emby | 795ms | 预估值（warmup 隧道基线，**从 runner 测**） |
| Emby → OneDrive 读文件头 | 7ms | 本次实测（已地板） |
| Emby → OneDrive 取直链 | 387ms | 本次实测（该条目此前播过 → 回看预热已把它取热；真正的**首次**取链是 0.4~1.3s 冷解析） |
| 你 → OneDrive 拉首字节 | 270ms | 预估值（也是 runner 侧读数） |
| 你 → OneDrive 拉流缓冲 | 测不到 | **剩下约 7 秒基本都在这里 + 播放器解码** |

判据：**直接打开通知里的 OneDrive 直链播放，同样要约 10 秒**（绕开 Emby 与 odlink 的
整条取链链路）⇒ 慢的主因是「客户端 → 微软 CDN」的建连/首字节/缓冲与播放器侧，
**仓库侧无解**；服务器侧代码实际只占 **0.39 秒**（7ms + 387ms）。

结论口径：通知里的「取直链」是唯一还会波动的服务器侧项，且它只在**冷解析**时大
（该条目**第一次**取链必冷 0.4~1.3s，取到即入 `dir_cache`/`link_cache`，之后重播
3~6ms；回看预热还会在每轮开头把最近播过的提前取好）。其余各段不是地板值就是
客户端侧，**不要再从服务器侧找这 8~10 秒**。
...
### 2026-09 优化批次成果存证（Phase 1–4 已收口，2026-09-15 定稿）

当年这轮优化的过程性文档（三方评估报告、分阶段实施计划、交接快照、用户实验清单）已随收官
从仓库根移除，成果全部落在本文各节；这里保留量化对比供追溯（before = 优化前基线 `216a9b8`）：

| 指标 | before | after | 落点 |
|---|---|---|---|
| install emby | 24m52s ~ 36m53s | **9m40s**（其中增量恢复 17~22min → **3m11s**） | §8 备份瘦身 |
| 运行期增量快照 | 2,823 文件 / 1,038MB | **157 文件 / 160MB** | §8 排除 metadata/library |
| 轮换空档 | 25–48min（cron 空档） | **≈11min**（自续接力） | §1 流式接力 |
| dir_cache 启动覆盖 | 每轮从 0 爬 | 装载 43,925 条（后全量回填 85,812） | §4.5 |
| 收尾 4 step | 自续触发 exit 127 | 全 success | §1 / §9 |
| 整轮总时长 | 5h39m（超 5h30m 红线） | 4h45m（270min 轮实测）；保留提至 300min 后 ≈5h20m | §1 并发与时长 |
| 起播 8–10 秒归因 | — | 服务器侧仅 0.39s，其余在「客户端 → 微软 CDN」路由与播放器 | 上节归因闭环 |

> **算轮换空档的坑**：中间夹 cancelled 轮时，朴素的前后差值会把人工调试 dispatch 的间隔算成
> 假空档（曾算出 64min）——必须把 cancelled 轮一起排进时间线，取「上轮 job 结束 → 下轮
> `run cloudflared` 就绪」的稳态对。
