...
| `EMBY_PREFETCH_MAX_DIRS` | 目录回填上限（默认 `400` 个目录）。配合并发可上调（800 → 1837），盯 `/stats` 的 `graph.err` |
...
> 调大 `EMBY_PREFETCH_MAX_DIRS` 前先看这条：回填 800 个目录 ≈ **4.4 万条 / 10MB**
> （`/stats` 的 `dirs_cached` 实测 43,925）。条目数超过 `ODLINK_DIR_CACHE_MAX`
> （默认 5 万）会按插入顺序丢最旧的，而最旧的恰好是最先回填、也最常被访问的
> 那批目录。要继续上调目录数，应同步把上限与这个文件带来的上传体积一起评估。
