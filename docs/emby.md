...
| `WW_KEEP_MIN` | 跨 run 海报档位保底数（默认 `5`）。半衰期衰减后若不足这个数，从衰减前的 Top-N 补足——避免长时间没人访问就把已学到的尺寸淘汰干净。补足的仍是历史实测过的尺寸，不是默认档 |
