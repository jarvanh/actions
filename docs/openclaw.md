...
> 「💳 账号池」取自 `workbuddy-gateway status` 的文本输出，**不解析** `workbuddy-status.json`
> （内部键名上游未文档化）；「剩余积分 / 免费模型 / 模型冷却」只在 `monitor` 的交互式表格里，
> 脚本不可取，故不展示。字段口径逐条见 [`telegram-notify.md`](telegram-notify.md) 2.5 节。
