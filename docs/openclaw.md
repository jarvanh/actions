...
- **完整性校验（取代缩水保护）**：打包前校验关键文件必须存在且非空 —— 主包是
  `.openclaw/openclaw.json`，zcode 是 `.zcode/cli/db/db.sqlite`（会话正文）与
  `.zcode/v2/tasks-index.sqlite`（侧栏列表）。缺失即判「状态不全」，
...
- **zcode 归档的入包口径**：`~/.zcode` 下 `cli/` 与 `v2/` 两棵子树整体入包，只排除
  `cli/log`（运行日志）与 `cli/plugins/cache`（13MB 插件包缓存，可重新下载）。
  `server/` 不入包 —— 它是桌面客户端每轮自动重新部署的运行时。
  > 早先的版本把 `cli/rollout`、`cli/exec` 当「调试转储」一并排除，等于切掉会话相关内容；
  > 同时漏掉整个 `v2/`，其中 `tasks-index.sqlite` 是侧栏会话列表（标题/置顶/归档/未读/删除）。
  > 这是 2026-09-18「备份不齐全、会话仍丢」的根因，已改为一律入包：宁可包大，不可少数据。
