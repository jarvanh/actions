...
| `TAIER_ALIVE_PROBE` | `1` | 测速前先测活（跳过连不上的节点，省下一个 ≈25 秒的测速窗口）。**默认开**：误杀风险由熔断（开头连续 8 个**真连不上**且无一成功即关掉探测）与 fail-open（探测机制出错按存活处理）兜住；⚠️ 「mihomo 不认识节点名」**不算**真连不上、也不再触发熔断，而是放回队尾重排（2026-09-23 改，见上「两道防线」）。run 34859505000 曾 27 个节点全 `Resource not found`（探测用的节点名在 mihomo 里对不上）属已知代价，要规避就 dispatch 选 `alive_probe=off`（或给本工作流传 `alive_probe: off`）；**不接仓库 Variables**。**这是本工作流唯一的准入关口**——节点收集层已不再按 provider 的 `alive` 预筛（见 [gitee 文档 · 为什么节点收集不等健康检查](proxy-speedtest-gitee.md#为什么节点收集不等健康检查)），收集来的节点全量进循环 |
...
1. **开测前等就绪**：`speedtest_gitee.wait_provider_ready()` 拿头尾各两个节点名探
   ...
   名字便宜）；超时后照常往下走（不阻塞整轮）。
   ⚠️ **超时按「mihomo 实际加载量」自动放大、不得写死 60 秒**：写死 60 秒在 2123 个节点上
   等不完（实测等满 60 秒 / `attempts=138` 全 404）；**按候选数算同样不够**——2026-09-23
   加载 20004、过滤后 1539，按 1539 只给 152 秒仍等不完（`attempts=322` 全 404）。
   等不完 ⇒ 下面的测活一开就熔断 ⇒ **上千个死节点没被提前筛掉、全跑满 ~16 秒的测速窗口**
   （1498 × 15.9s ≈ 6.6h；1309 × 16.5s ≈ 6h，单这一项吃掉整个 5 小时预算）。
   公式（现在是 `60 + 0.3 × 加载量`，封顶 300）与边界见
   [gitee 文档 · 为什么开测前要等 provider 展开](proxy-speedtest-gitee.md#为什么开测前要等-provider-展开三套共用)。
   ⚠️ **等不到是常态，不是异常**：`provider_ready`（成功）事件在**所有**历史轮次的日志里
   都**从未出现过**，给到 900 秒那轮仍是 `waited=900.34 attempts=1810` 全 404。所以
   **展开等待超时 ⇒ 直接关掉测活、全量放行去测速**（与熔断同口径，但早得多、且不动判死
   判据）——否则每个节点各吃一次 `Resource not found`（实测 4743 次）纯空转。
   **它是共享层的，三套都调**
   （taier 在 `nodes_collected` 之后、gitee 在 `provider_snapshot_ready` 之后、
   cdn 在 `nodes_collected` 之后），留在任一套的本地副本都算回归——测试第 9 组用
   对象同一性 + 调用点存在性双重钉住，第 14 组钉住「不写死 60」且「都传 `total_loaded`」。
2. **`Resource not found` 不参与判死**：`is_unknown_proxy_error()` 把它与「真连不上」分开。
   ⚠️ 这类**不再停用测活层**，而是把节点**放回队尾重新排队**（2026-09-23 改）：
   mihomo 加载上万个节点时展开极慢，此刻「不认识」是**加载状态**而非节点属性；旧逻辑
   「连续 8 个不认识就永久关掉测活」会把中间态当成永久故障 ⇒ 上千个死节点再没人拦。
   放回队尾等于让本轮其余节点先走，天然形成「等一会儿再试」，不需要额外 sleep；
    累计上限取「候选数 × 2」（给展开留两轮完整机会），**到顶后必须放行去测速**。
    ⚠️ 2026-09-23 零产出事故：到顶分支曾写成 `continue` 直接跳过 ⇒ 1581 个候选一个都没测
    （`taier_speedtest_done node_count=0`）。fail-open 的语义是「照测」不是「不测」——
    重排是「晚点再试」，试不出来就按老办法照测。测试第 15d 条钉住这一点。
   **真连不上**的熔断（连续 8 个真死且无一成功 ⇒ `_probe_enabled = False`）原样保留，
   只放开了「不认识」这一类——测试第 15 组分别钉住这两条。
