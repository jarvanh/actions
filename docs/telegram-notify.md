...
| 2 | 结论值（kv 值 / 状态 / 计数 / 数值） | `<b>` | `tg_add_kv` | `状态：<b>成功</b>` |
...
| 5 | 元数据（` · ` 后的大小 / 时间 / 图例 / 备注） | `<i>` | 手写 | ` · <i>1.7 GB · 2160p</i>` |
| 6 | 折叠行 | `<i>` | `tree_code_fold` | `<i>还有 8 条…</i>` |
| 7 | 说明段（独立成段） | `<i>` | `tg_add_note` | `<i>密码 SSH/RDP 共用</i>` |
| 8 | 日志块 | `<pre>` | `tg_add_block` | `<pre>…</pre>` |
| 9 | 可复制命令块 | `<pre>` | `tg_add_block`（§2.3） | `<pre>gh workflow run …</pre>` |

**裁决规则（冲突时按此，勿各自发挥）**：

1. **kv「值」位置不得出现 `<i>`**：`<i>` 只作元数据/说明。说明性内容（有效期、连接方式…）要么进 `<b>`，要么降为 `tg_add_note` 说明段。
...
4. **组头计数在粗体外**：`<b>名</b> · N`（不是 `<b>名 · N</b>`）。
5. **分节前缀不得自造**：`🥇` / `⭐` 等不在语义表内的前缀一律不用；名次类分节用 `<b>🏆 最快节点 · N</b>`，图例挂 ` · <i>…</i>`。
6. **条目不编号**：树形条目靠顺序表名次，不加 `1.` `2.`（与 bash 侧一致）。
7. **emoji 一律在 `<b>` 内，无例外**（2026-09-09 拍板）：标题、分节、组头、条目主体、
   计数行的字段图标全部如此——`<b>🎬 片名</b>`、`<b>📁 组头</b>`、`<b>✅ 3</b>`，
   不是 `🎬 <b>片名</b>` / `📁 <b>组头</b>` / `✅ <b>3</b>`。
   emoji 是彩色图形字形，粗体不改变其外观，内外观感相同；统一进 `<b>` 是为了与
   `tg_add_title` / `tg_add_section` / `tg_add_kv` 真源函数的整体包裹形态一致、规则零歧义。
   （`🔗` 收尾链接在 `<a>` 内、`▸`/`▶` 动作行前缀在 `<i>`/`<a>` 内——都不是 `<b>` 场景，不涉本条。
   `<i>` 说明段（`tg_add_note`）内的 emoji **随段裸置是既定形态，不是违例**：
   `tg_add_note` 对整段做 `escape_html`，段内无法携带任何 HTML 标签（往里传 `<b>` 会
   渲染成字面文本 `&lt;b&gt;`，2026-09-09 实测踩坑）。pwsh 手拼说明段不受此限。
   唯一其它豁免：等宽对齐块——进度面板阶段行/批次历史行整行包 `<code>` 换数字列
   竖向对齐，emoji 与 `▸` 随之在 `<code>` 内（§4 字段表）。）
...
kv 值一律 `<b>`（语义表 #2），` · ` 后的元数据 `<i>`，emoji 入 `<b>`（裁决 7）；
...
🎬 五十度灰 (2015)
━━━━━━━━━━━━━━━━━━
规格：<b>电影 · 2 小时 08 分 · 1080p · h264 · 5.5 GB</b>
链路：<b>⚡ 302直连 OneDrive</b> · <i>直链剩余 40 分钟</i>
客户端：<b>203.0.113.57 · Safari · macOS</b>

<b>⏳ 起播等待</b>
  ├─ <code>你 → Cloudflare</code> · <i>27 毫秒</i>
  └─ <code>你 → OneDrive 拉流缓冲</code> · <i>这段服务器测不到</i>
...
| emoji 在 `<b>` 外（手拼 HTML） | `✅ <b>3</b>`、`📁 <b>组头</b>` | `<b>✅ 3</b>`、`<b>📁 组头</b>`（裁决 7，无例外） |
...
3. 进度面板阶段行/批次历史行整行 `<code>` 等宽包裹（emoji/`▸` 随之在 `<code>` 内）——
   换计数列竖向对齐，见上方字段表。
4. `🔗` 在 `<a>` 内、`▸`/`▶` 动作行前缀在 `<i>`/`<a>` 内——链接/动作形态，非 `<b>` 场景。
5. `<i>` 说明段（`tg_add_note`）内 emoji 裸置——该助手对整段 `escape_html`，
...
- [ ] 列表分节计数 ` · N`（分节后跟条目列表必带；kv/单段说明分节不带）；条目元数据 ` · <i>…</i>`；kv 行全角冒号
