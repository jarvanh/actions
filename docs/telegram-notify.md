...
| ⚠️ glm-proxy 已启动 · 上游不可用 | `openclaw.yml` | 已监听 8787 但自检消息未收到有效回答（上游配额耗尽 / 限流 / 凭据无效） |
...
原因：上游配额耗尽（1113 · 余额不足或无可用资源包）· 充值或等配额重置后自动恢复
...
在监听，上游答不答得上是另一件事。两者并成一个 `OK` 时，上游配额耗尽（服务本身
完全正常、只需等）会被报成 ❌ 启动失败，读者就去排查一个没坏的进程。分法：
...
「余额不足或无可用资源包」也包在 HTTP 429 里返回（错误码 `1113`），此时凭据其实还在
生效，笼统写会让读者白换一轮 key。按日志尾部匹配，顺序在前的先命中：

| 日志特征 | 写出 |
|---|---|
| `1113` / `余额不足` / `无可用资源包` | 上游配额耗尽 · 充值或等配额重置后自动恢复 |
| `-> 401` / `-> 403` / `invalid api key` | 上游凭据无效或已过期 · 需更换凭据文件 |
| `超时` / `timeout` / `ETIMEDOUT` | 上游请求超时 |
| `429`（前几项都不命中时） | 上游限流（429）· 稍后自动恢复 |
