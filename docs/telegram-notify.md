...
批次计数 emoji 字段表（进度面板批次历史行，**全字段恒显**，行宽预算 ≈31 全角/单行）：
`❌#n` 状态+批次号 | `✅n` 成功 | `🔧n` 修复 | `❗n` 失败（不用 ❌，避免与状态撞形）|
`⏭️n` 跳过 | `♻️n` 已有（目标端已存在）| `⏱时长`（无空格，如 `⏱1分15秒`）| `⬆️量`（rclone GiB，0 → `⬆️0 B`）。
✅/⚠️/❌/⏭️/♻️ 整批性质批次全部入史（MAX=6 滚动窗口，全量在运行日志）。
