...
<b>📁 组头</b> · 大小            ← 分组列表：组头路径加粗（emoji 同入 <b>，计数在粗体外）
...
7. **emoji 随所在区块的标签走**（2026-09-09 拍板，覆盖旧「一律进 `<b>`」）：
   - 标题 / 分节 / 组头 / 条目主体（非文件值）用 `<b>`，emoji 同入 `<b>`：
...
| emoji 在 `<b>` 外（手拼 HTML） | `✅ <b>3</b>`、`📁 <b>组头</b>` | `✅ 3`、`<b>📁 组头</b>`（裁决 7，无例外） |
