...
- **每日签到段（2026-09-26 新增）**：签到是 serve 内置后台协程，**无 CLI 子命令、
  ...
  | 结果列修正 | 以状态接口为准：`active=false` → 「活动未开启」（网关把 `code=10001` 一律幂等成功，国际站活动没开也被记成「今日已签」）；网关判「失败」保留原样（HTTP/原因在原始输出里更具体） |
  | 汇总行 | 仅状态接口**全覆盖**时按展示行重算（已签/未开启/未签/失败/跳过 + 今日合计 +N）；有查询失败退回网关日志口径，不硬凑 |
