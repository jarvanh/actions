...
| ❌ glm-proxy 启动失败 | `openclaw.yml` | 进程未在 8787 监听（脚本/代码缺失、启动脚本非 0） |
| ⚠️ glm-proxy 已启动 · 上游不可用 | `openclaw.yml` | 已监听 8787 但自检消息未收到有效回答（账户欠费 / 额度用尽 / 限流 / 凭据无效） |
| 🟢 glm-proxy 已就绪 | `openclaw.yml` | 自检消息已收到回答（仅首轮推） |
...
**示例：glm-proxy 已启动 · 上游不可用**（`openclaw.yml`，降级态）

```
⚠️ glm-proxy 已启动 · 上游不可用
━━━━━━━━━━━━━━━━━━
结论：已监听 8787 · 自检消息未收到有效回答
原因：账户余额不足或已欠费（1113）· 需充值，等待不会自动恢复
接口：<code>http://127.0.0.1:8787/v1</code>
代码目录：<code>/dropbox/self-hosted/glm-proxy</code>

服务本身正常，上游恢复后无需重启即可继续使用

日志尾部
<pre>POST /v1/messages bridge -&gt; 429 788ms {"error":{"code":"1113","message":"余额不足或无可用资源包,请充值。"}}</pre>

⏱ 已运行 4 分钟 · 🔗 运行日志
```

**本地反代的「起没起来」与「能不能用」必须分成两条判据。** 反代进程活着只说明它
在监听，上游答不答得上是另一件事。两者并成一个 `OK` 时，上游欠费/额度用尽（服务本身
完全正常，只需充值或等重置）会被报成 ❌ 启动失败，读者就去排查一个没坏的进程。分法：

| 判据 | 含义 | 通知 |
|---|---|---|
| 进程未在端口监听 | 脚本/代码缺失、启动即退 | ❌ 启动失败 |
| 已监听 + 自检未收到有效回答 | 上游不可用（多为暂时状态） | ⚠️ 已启动 · 上游不可用 |
| 已监听 + 自检通过 | 全通 | 🟢 已就绪（仅首轮推） |

**上游原因必须细分到「读者能照做」，不能笼统写「上游 429/凭据过期」。** BigModel 把
一大串不同性质的错误都包在 HTTP 429 里返回，靠裸 `429` 无法区分——**尤其「余额不足」
与「额度用尽」是两件事，处置完全相反**：

| 业务错误码 | 官方语义 | 通知里的「原因」 | 该做什么 |
|---|---|---|---|
| `1309` | GLM Coding Plan 套餐已到期 | 套餐已到期 · 需续订后恢复 | 续订 |
| `1311` | 当前套餐未开放该模型权限 | 套餐未开放该模型权限 · 换套餐或换模型 | 换模型/套餐 |
| `1313` | 不符合公平使用策略 | 触发公平使用策略限流 · 需申请解除 | 去个人中心申请 |
| `1308` / `1310` / `1316`… | 已达使用上限 · 限额将在 `next_flush_time` 重置 | 已达使用上限 · 限额到点自动重置，等待即可 | **等** |
| `1113` | **您的账户已欠费，请充值后重试** | 账户余额不足或已欠费 · **需充值，等待不会自动恢复** | **充值** |
| `-> 401` / `-> 403` / `invalid api key` | 身份验证失败 | 上游凭据无效或已过期 · 需更换凭据文件 | 换凭据 |

匹配顺序上「使用上限」必须排在 `1113` **之前**：`1316` 的官方文案是「已达到 5 小时使用上限。
**主账号余额不足**，无法使用超额按量付费……」，含「余额不足」字样但归属配额类，
先匹配才能避免误判成欠费（否则会让人白充一次钱）。码表见
[智谱错误码文档](https://docs.bigmodel.cn/cn/api/api-code)。

> 「额度耗尽」与「余额不足」不能互换：Coding Plan 官方明确**额度耗尽不会消耗账户余额**，
> 反过来说明 `1113` 不是额度问题而是账户没钱。写成「配额耗尽 · 等重置后自动恢复」
> 会让人干等一场——欠费等不来自动恢复。
...
- **鉴权一行固定写「需 API Key」**：serve 以 `-addr 0.0.0.0` 监听（对 Tailscale/局域网
  开放），故启动时显式传 `-api-key` 做客户端 Bearer 校验，取值与 glm-proxy 同为
  `AI_GATEWAY_API_KEY`。**不得编造密钥值**，通知里只写「需 API Key」这一形态，不回显 key。
  （更早前本节写「仅回环监听，无需密钥」，与 `-addr 0.0.0.0` 的实际启动参数不符，已订正）
