...
3. **条目前缀**：有分组层级用 `├─/└─` 树形。真源只提供树形助手（`tree_lines` /
   `tree_code_fold`），**新增通知一律走助手出树形**；`• ` 平铺是历史遗留形态
   （`file_restore.sh` / `sync_marker.sh` / `file_split.sh` 既有段落），仅限原地保留、
   不新增。**同一条通知内两种前缀不得并存**（2026-09-09 起，`sync_notify.sh` 排除规则
   已从 `• ` 改回树形）。
...
7. **emoji 一律在 `<b>` 内，无例外**（2026-09-09 拍板）：标题、分节、组头、条目主体、
   ...
   两个既定豁免：① 等宽对齐块——进度面板阶段行/批次历史行整行包 `<code>` 换数字列竖向
   对齐，emoji 与 `▸` 随之在 `<code>` 内（§4 字段表）；② `<i>` 说明段内的 emoji 随段保持
   原样即可，不强制再包一层 `<b>`（已写嵌套的也不回退）。）
...
2. **多套同类信息** —— 条目虽少但存在多套并列结构（如 SSH / RDP 两套入口凭据），
   ...
   （这是「分节计数一律 ` · N`」的**唯一例外**——条目数少且一眼可数，计数行反而噪音；
...
- [ ] 分节计数 ` · N`（多套同类信息分节例外，见 §2.2 场景 2）；条目元数据 ` · <i>…</i>`；kv 行全角冒号
