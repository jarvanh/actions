...
> 行尾括号后缀是既有写法（`（预估值）`）。`取直链` 行的括号标**本次取链的性质**：
> `（缓存命中）`= 直链缓存直接给出（等于没花时间）、`（冷解析）`= 临时解析/签发
> （真花了这段时间）。判据优先取 odlink 的真实缓存命中标记，取不到时按耗时阈值
> 兜底（`EMBY_LINK_HOT_MS`，默认 200 毫秒：热命中 3~6 毫秒、冷解析 0.4~1.3 秒）；
> **两者都取不到时省略后缀，不猜**。括号内容是自然语言，裸文本，不套 `<code>`。
