...
7. **emoji 一律在 `<b>` 内，无例外**（2026-09-09 拍板）：标题、分节、组头、条目主体、
   ...
   `<i>` 说明段内的 emoji 同样入 `<b>`（`<b>⏭️</b> 已跳过…`），2026-09-09 起不再有裸置豁免。
   唯一豁免：等宽对齐块——进度面板阶段行/批次历史行整行包 `<code>` 换数字列竖向对齐，
...
既定豁免清单（**技术必需**，不算违例，勿"修复"；2026-09-09 收敛后仅剩这 4 条，
风格类豁免——`• ` 平铺、说明段 emoji 裸置、列表分节免计数——已全部清除）：
