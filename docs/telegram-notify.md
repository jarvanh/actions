...
🚨 两端均无 · 1
  └─ <code>media/视频D.mp4</code> · 目标端副本与源端均不存在

目标端副本与源端都不存在，条目已保留未出账，需人工确认

⏸ 跳过待确认 · 1
  └─ <code>media/视频E.mp4</code> · 原路径已存在但大小不符或未记录大小（已有 1024B / 期望 4096B）— 未覆盖，需人工确认

原路径已有同名文件但大小不符，未覆盖，条目保留

❌ 失败清单 · 1
  └─ <code>media/视频C.mp4</code> · 目标端写入被拒

已还原与已失效（源端仍在）条目已从 marker 修复清单移除；其余保留，可重试。

⏱ 已运行 5 分钟 · 🔗 运行日志
```

> 四态口径（2026-09-26）：**已还原**（复制回原路径，保留备份副本）/ **已失效出账**
> （替代副本已不存在但源端仍在 ⇒ 判失效、出账）/ **跳过待确认**（原路径已有同名文件
> 但大小不符，不覆盖、条目保留）/ **失败**（真出错，可重试）。另有 **🚨 两端均无**
> 独立成段：目标端副本与源端都不存在时才出现，是真风险，条目保留不出账。
...
- **每日签到段（2026-09-26 新增）**：签到是 serve 内置后台协程，**无 CLI 子命令、
  不写快照**；网关还把签到接口响应体整个丢掉（`checkinAccount` 只接 status/err），
  日志只有「成功/已签」没有积分，「已签」幂等响应（`code=10001`）也不带积分。
  积分真源是独立的活动状态接口（只读、幂等、无副作用，逐账号实测 200）：
  `POST {Origin}/v2/billing/meter/checkin-activity-status`，`data` 含 `active` /
  `today_checked_in` / `today_credit`（本次）/ `total_credits`（累计）/
  `streak_days`（连签）/ `daily_credit`（每日可得）。字段口径：

  | 展示 | 来源与条件 |
  |---|---|
  | 条目行 `凭据 · 站点 · 结果 · 本次 +N · 累计 N · 连签 N 天` | 积分三项仅 `active=true` 且已签时拼接；查询失败/无数据只展示结果，绝不编造积分 |
  | 子行 `Buddy 旅行 +N` | 仅日志落过 `[Travel] … 旅行奖励领取成功（credit=N）`；窗口=**今天东八区全天**（奖励一天领一次，常在更早轮次，按最后一轮会漏） |
  | 结果列修正 | 签到活动开启（`active=true`）→ 积分口径最优先；网关判「失败」保留原样（HTTP/原因在原始输出里更具体） |
  | 国际号活跃口径（2026-09-26 改造） | 国际站**没有签到活动**（积分按「每日活跃对话」发放，次日凌晨到账，见 autumnsentiment/workbuddy-helper 逆向结论）。启动步逐国际号发一发最小活跃请求（免费模型 + max_tokens=64 + 客户端特征头），HTTP 200 且响应确为对话流才落当日标记（`活跃 ✓ 已触发`）；429 限流按响应自带恢复时刻落 defer 文件（到期自动重试），显示「活跃待重试 · 限流 HH:MM 后自动重试」。**绝不预填到账数字**（是否 +30 以后端入账为准） |
  | 汇总行 | 仅状态接口**全覆盖**时按展示行重算（已签/活跃/待重试/未开启/未签/失败/跳过 + 今日合计 +N）；有查询失败退回网关日志口径，不硬凑 |
...
- **Plan 通道降级如实报**：主探 `glm-5.3-flash`（Plan 通道，需无痕验证码，
  ...
  （API Key 通道，免验证码；**网关侧默认关闭**——meta 设置
  `bigmodel_channel_enabled` / env `ZCODE_BIGMODEL_CHANNEL` / 后台「设置」可开，
  关闭时退探即时拿到 403 channel_disabled，不会像欠费未关时那样挂起约 2 分钟）。
  退探能通 = 服务本体健康、仅 Plan 通道降级：
