...
**trae2api 通知的字段口径**（TRAE SOLO 反代，7864；`openclaw.yml`，`trae_notify` 与
`wb_notify` 同构）——版式完全同 workbuddy-gateway，差异只在数据来源：

- 标题同形：`🟢 trae2api 已就绪` / `⚠️ trae2api 已就绪 · 签到工具执行失败` /
  `❌ trae2api 启动失败`。就绪状态在标题里，「结论」只留签到汇总
  （`已签 X · 跳过 Y · 失败 Z（共 N）`）。
- 无「版本 / 更新」行：上游无 release（源码浅克隆重建镜像）。
- 「🔑 凭据」列 auths 目录的账号文件名（与 workbuddy 同口径：只列文件名、绝不回显
  内容——真实会话 Token），仅在就绪态出现。
- 「💳 账号池」来自批量签到输出：条目 `uid · 昵称 · 状态`（同 wb_notify 的
  「凭据文件 · 站点 · 状态」），数字与时刻下沉子行——子行 1 `积分 N`，其后是最早
  过期的 3 个积分包（`剩 N 分 · 时刻 过期`）。条目计数用 `grep -c .` 数非空行：
  `wc -l` 会把尾空行算进去，`_i` 永远追不上 `_total`，末条 `└─` 轮空、整棵树全是 `├─`。
- 「鉴权」写「需 API Key（面板/CLI 共用）」——key 不回显（与 glm-proxy 同口径；
  trae2api 的 key 是独立的 TW2A_API_KEY，不与 AI_GATEWAY_API_KEY 同值）。
- 「🧾 原始输出」**仅失败/降级态渲染**（同 wb_notify 的 $6 口径）：启动失败给容器
  日志尾部、签到工具失败给签到 stderr 尾部（均 1200 字节）；**成功通知不带**——
  签到工具会把 `checkin done: 0/4 ok` 这类汇总行写进 stderr，照搬会把正常轮渲染出
  一段像报错的「原始输出」。
- 通知只从 openclaw.yml 发（曾有独立 workflow trae2api-notify.yml 走第二条链路，
  已删除）。
