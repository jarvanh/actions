...
| 5 | `⏱mm:ss`（定宽补零） | 进度面板批次行 | `⏱01:15` |
  ...
  - 第 5 层是**面板列字段**（为了计数列竖向对齐），与第 1–4 层不是一套体系，见 §5.2；
...
|---|---|---|---|
| 手拼 HTML（`msg+="<code>…"`） | pwsh 无助手；bash/python 已全部走助手 | 用 §8.1 助手：`tg_add_*` / `tg_entry` 家族 / `tg_add_pre`（pwsh 按 §4 形态手拼、值经 `Esc-Html`） | `tg_entry "video.mp4" "1.7 GB"` → `<code>video.mp4</code> · 1.7 GB`（再交 `tree_lines` 出 `  ├─ ` 前缀） |
...
| 英文紧凑时长（`5h 57m`） | 脚本内部格式直接输出 | 中文三段式（§4.3 时长五层） | `⏱ 已运行 5 小时 57 分` |
| 高精度浮点（`2.16068914 秒`） | 原始值直出 | 两位小数（§4.3 时长五层） | `起播 2.16 秒` |
