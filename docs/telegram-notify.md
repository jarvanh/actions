...
```
{emoji} <b>标题</b>              ← tg_add_title（自动拆前导 emoji 到标签外，文字加粗）
━━━━━━━━━━━━━━━━━━              ← TG_SEP（18 个全角横线，勿手写；其后不空行）
标签：值                          ← tg_add_kv（全角冒号；值不加粗）
标签：<code>路径/命令</code>      ← tg_add_path（等宽展示）

{emoji} <b>分节 · N</b>          ← tg_add_section（自动拆 emoji；段前空行；计数一律 " · N"）
{emoji} <b>组头</b> · 大小      ← 分组列表：组头文字加粗（emoji 与计数在粗体外）
...
`§2` 模板只列了 5 类形态，实际内容有 9 类语义——缺归属的部分过去由 4 套发送层各自手拼，
这是全库标签不一致的根源。下表是唯一裁决依据：

> **2026-09-09 拍板：去掉 ``，结论值不再加粗。**
> `<b>` 只保留在「标题 / 分节 / 组头 / 条目主体（非文件值）」；kv 值、状态、计数、
> 数值、元数据、折叠行、说明段一律**裸文本**（动态内容照常转义）。
> 理由：全行加粗 + 斜体混排让通知显得花，且中文斜体效果不明显；靠缩进、全角冒号、
> ` · ` 分隔已足够分层，`<code>` 等宽承担「可复制」的视觉区分。

| # | 语义 | 标签 | 产出方式 | 例 |
|---|---|---|---|---|
| 1 | 标题 / 分节 | `<b>` | `tg_add_title` / `tg_add_section` | `📋 <b>任务预览 · backup</b>`、`📊 <b>同步对 · 2</b>` |
| 2 | 结论值（kv 值 / 状态 / 计数 / 数值） | 无 | `tg_add_kv` | `状态：成功` |
| 3 | 机器值（路径 / 文件名 / 命令 / 密码 / IP / ID / 端口 / 原始异常串） | `<code>` | `tg_add_path` | `文件：<code>a/b.mp4</code>` |
| 4 | 条目主体 | 文件类 `<code>`，其它 `<b>` | `tree_code_fold` / 手写 | `<code>x.mp4</code>`、`<b>香港 01</b>` |
...
1. **kv「值」位置不加任何标签**：值一律裸文本 + 转义（加粗/斜体都不用）。说明性内容
   （有效期、连接方式…）要么进 kv 值，要么降为 `tg_add_note` 说明段。
2. **条目非文件值不得裸文本**：可复制的用 `<code>`，其余用 `<b>`。
...
4. **组头计数在粗体外**：`{emoji} <b>名</b> · N`（不是 `<b>{emoji} 名 · N</b>`）。
5. **分节前缀不得自造**：`🥇` / `⭐` 等不在语义表内的前缀一律不用；名次类分节用 `🏆 <b>最快节点 · N</b>`，图例挂 ` · …`。
6. **条目不编号**：树形条目靠顺序表名次，不加 `1.` `2.`（与 bash 侧一致）。
7. **emoji 一律不入 `<b>`**（2026-09-10 拍板，最终裁决；覆盖 09-09 的「同入」）：
   `<b>` 只包文字——`🎬 <b>片名</b>`、`📁 <b>组头</b>`、`⚠️ <b>上传失败</b>`，
   不是 `<b>🎬 片名</b>` / `<b>📁 组头</b>`。
   理由：`tg_add_note` 对整段转义、段内无法带标签（见下），「emoji 入 `<b>`」在说明段
   路径天然不可行；emoji 是彩色图形字形，入不入 `<b>` 外观无差，与其按区块区分两种
   写法，不如统一为「永不入 `<b>`」零歧义。
   `tg_add_title` / `tg_add_section` 已内建**前导 emoji 自动拆分**（perl，覆盖
   U+2100-2BFF / 1F000-1FAFF / VS16 / ZWJ），调用方仍传「emoji + 短语」整串、无需感知；
   手拼处照此形态。
   - 标题 / 分节 / 组头 / 条目主体（非文件值）的文字用 `<b>`：
     `🎬 <b>片名</b>`、`📁 <b>组头</b>`、`🏆 <b>最快节点 · 5</b>`，不是 `<b>🎬 片名</b>`。
   - **计数/数值/kv 值一律无标签**，emoji 也随之裸置：`✅ 3`、`📊 总 3 · 待处理 1`——
     这些是数据行，不是条目主体。
   - 说明段（`tg_add_note`）是裸文本段（该助手对整段 `escape_html`，段内无法携带
     任何 HTML 标签），emoji 自然裸置。
   - 等宽对齐块：进度面板阶段行/批次历史行整行包 `<code>` 换数字列竖向对齐，
     emoji 与 `▸` 随之在 `<code>` 内（§4 字段表）。
   - `🔗` 收尾链接在 `<a>` 内——链接形态，非 `<b>` 场景。
8. **错误/原因三选一**：中文阶段/原因 → `<b>`；原始异常串 → `<code>`；多行日志 → `<pre>`。
...
kv 值无标签（语义表 #2），` · ` 后的元数据同样无标签，emoji 随所在区块走（裁决 7）；
...
`server` 解析失败时该块降级为「归属获取失败」一行（`<b>`，不裸文本）。
...
| emoji 入 `<b>`（2026-09-10 起反转） | `<b>✅ 3</b>`、`<b>📁 组头</b>` | `✅ 3`（计数无标签）、`📁 <b>组头</b>`（裁决 7，无例外） |
...
既定豁免清单（**技术必需**，不算违例，勿"修复"；2026-09-09 收敛后仅剩这 5 条，
风格类豁免——`• ` 平铺、列表分节免计数——已全部清除）：
...
3. 进度面板阶段行/批次历史行整行 `<code>` 等宽包裹（emoji/`▸` 随之在 `<code>` 内）——
   换计数列竖向对齐，见上方字段表。
4. `🔗` 在 `<a>` 内、`▸`/`▶` 动作行前缀在 ``/`<a>` 内——链接/动作形态，非 `<b>` 场景。
5. `` 说明段（`tg_add_note`）内 emoji 裸置——该助手对整段 `escape_html`，
   段内无法携带 HTML 标签（见裁决 7 注）。
...
- [ ] 列表分节计数 ` · N`（分节后跟条目列表必带；kv/单段说明分节不带）；条目元数据 ` · …` 无标签；kv 行全角冒号、值不加粗
