...
- **调试轮用 `run_minutes` 短轮，不要靠取消来观察收尾**：手动 dispatch 可传 `run_minutes`
  （如 3）让整轮几分钟内自然走完收尾，从而看到完整的接力判定输出。**取消轮次观察不到
  这条路径**——取消时 `Start-Sleep` 所在的步骤被直接终止，虽然收尾步骤带 `if: always()`
  仍会执行，但那是 cancelled 分支；要验证 success 路径的派发必须让轮次自然结束。
  `emby.yml` 同名入参同义。

- **自续触发不携带本轮入参**（2026-09-15 补）：接力 step 调 `gh workflow run <wf>`
  时**只带事件类型、不带 `-f` 参数**，所以被接力的下一轮一律用**声明里的默认值**
  （例如 `openlist.yml` 的 `sync_budget_min=320`）。影响: 手动用 `sync_budget_min=60`
  起一轮"短轮"做验证后，自续出来的下一轮会回到 320min（不是短轮链）。
  需要连续短轮就手动 dispatch；**不要为此外改接力 step**（`openlist.yml` 的 §8 红线 4
  明令禁止改动自续触发逻辑——接力链本身已验证健康）。
