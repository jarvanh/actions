...
1. **wopan176 的 OpenList 驱动登录令牌（8005 的根因）** —— 2026-09-14 用户答「账号状态正常」，
   但 **8005 不是账号状态**：它是 OpenList 里 wopan176 存储配置的**驱动登录令牌**失效
   （OpenList 把驱动层 8005 包装成 HTTP 405 返回给 rclone，run 日志里只看得到 `405 Method Not Allowed`）。
   两条证据链指向它：
   ...
   **需要用户做的**：登 OpenList 管理面 → 存储 → wopan176 → 编辑 → 重新获取 token。
