...
### 11.12 管线利用率：一轮 21% 花在盲等上（2026-09-14 静默间隔分析）
...
**未做（下一轮候选，需单独设计）**: 缓存刷新那 42min 的主体是两次全量 `rclone size`。
缩短/去掉会直接落入"读太早 ⇒ 假阴性"这一类问题（本日已修的折叠校验、探针可见性都是
这个成因），**不能盲改**；可行方向是改用 OpenList API 的缓存计数替代一次全量 size，
但要先确认该计数在刷新后何时可信。
